	...
	String url="jdbc:odbc:Test2";
 	...
 	String pwd=new String(request.getParameter("pwd")).trim();//密码
		...
		if(rs.next()){
			...
			out.println("<script>alert('登陆成功'); window.location='index.html';sessionStorage.setItem('username',"+username+");</script>");//修改跳转页面
		}
		else{
			out.println("<script>alert('登陆失败,请检查用户名密码');window.location='login.html'; </script>");///////////
		}
           	rs.close();
            	st.close();
            	con.close();
